...
wandb.init(project="dlcv_4-1_ProtoNet")

class ProtoNetConfig:
    def __init__(self, **kwargs):
        self.required_key = ('epochs', 'learning_rate', 'train_way', 
                        'val_way', 'shot', 'query')            
        
        self.attr = self._ensure_cfg(kwargs)
    ...
    def _ensure_cfg(self, cfg):  
        for a in self.required_key:
            #print(f"{a} in cfg:", cfg.get(a, None))
            assert cfg.get(a, None), print(f"Argument {a} not given.")
...
class ProtoNetTrainer:
    ...
    def __init__(self, model, cfg=None):
        ...
        self.cfg_str = f"{self.val_way}way{self.shot}shot_{cfg['distance']}"
    ...
    def train(self, data_loaders, dest_dir):
        ...
        timer = Timer()
        logger = TrainingLogger(dest_path=os.path.join(dest_dir, "log", f"epoch_log_{self.cfg_str}.txt"))
        logger.print(
        ...
        while self.epoch < self.num_epoch + 1: 
            ...
            for i, batch in enumerate(train_loader, start=1):
                ...
                logits = self.model(img[:len_support], img[len_support:], num_way=self.train_way) # (query, way)                 
                label = make_label(label[:len_support], label[len_support:], self.train_way)
                
                # label = torch.arange(self.train_way).repeat(self.query).to(self.device)
                # label2 = label2.to(self.device)
                # print("Equal:", (label == label2).sum()/len(label))
                # return 
                # print("logits:", logits)
                # print("label[:len_support]:", label[:len_support])
                # print("label[len_support:]:", label[len_support:])
    ...
    def meta_test(self, test_loader, has_gt=False):
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_loader, start=1):
                ...
                logits = self.model(shot, query, num_way=self.val_way) # (query, way) 
                predictions.append(logits.argmax(1).tolist())
                ...
                if i % 10 == 0: 
                    print('Inferencing: {:3d}/{:3d}' \
                    .format(i, len(test_loader)) + ' '*10, end='\r')
