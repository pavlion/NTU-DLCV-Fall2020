...
def main():
    ...
    dest_dir = os.path.join("ckpt", "pb3.1")
    _ensure_dir(os.path.join(dest_dir, "log"))
    
    trainer = ProtoNetTrainer(model, train_cfg)
    trainer.cfg_str = f"{train_cfg['val_way']}way{train_cfg['shot']}shot_M{train_cfg['M']}_IFI"
    trainer.train([train_loader, val_loader], dest_dir=dest_dir)
