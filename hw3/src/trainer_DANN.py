...
class P3Trainer(Trainer):    
    ...
    def __init__(self,
        ...
        self.saved_model_name = f"best_loss_{self.min_loss}"
        ...
        with torch.no_grad():
            if test_loader is None:
                samples = self.model.sample(
                    ...
                    device=self.device
                ).permute(0, 2, 3, 1).cpu() # [num_samples, 3, H, W]->[num_samples, H, W, 3]
                generated_imgs = [sample.numpy() for sample in samples]
                print("Finish generating")

            else: # test_loader is given
                generated_imgs = {'imgs': [], 'loss': [], 'file_name': []}
