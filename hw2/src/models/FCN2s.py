...
class FCN2s(nn.Module):
    def __init__(self, num_class, dropout=0.1):
        ...
        self.upsample_2 = nn.ConvTranspose2d(num_class, num_class, kernel_size=2, stride=2) # [-1, num_class, 14, 14]

        # for pool1
        self.score1 = nn.Conv2d(64, num_class, kernel_size=1) # [-1, num_class, 28, 28]
        self.upsample_1 = nn.ConvTranspose2d(num_class, num_class, kernel_size=2, stride=2) # [-1, num_class, 14, 14]
    ...
    def forward(self, x):
        pool1 = self.features1(x)       # [-1, 64, 112, 112] 
        pool2 = self.features2(pool1)   # [-1, 128, 56, 56] 
        pool3 = self.features3(pool2)   # [-1, 256, 28, 28] 
        ...
        # fcn4s = upsampling fcn4s_pre 4x
        
        fcn2s_in1 = self.upsample_2(fcn4s_pre) # [-1, num_class, 112, 112]
        fcn2s_in2 = self.score1(pool1) # [-1, num_class, 112, 112]
        fcn2s_pre = fcn2s_in1 + fcn2s_in2 

        fcn2s = self.upsample_1(fcn2s_pre)  # [-1, num_class, 224, 224]
